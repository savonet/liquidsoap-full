...
```liquidsoap
settings.request.metadata_decoders.set(["FFMPEG", "FLAC", "MP4", "OGG", "TAGLIB", "ID3V2"])
```
...
```liquidsoap
settings.protocol.replaygain.path.set("/usr/share/liquidsoap/2.0.7/bin/extract-replaygain")
```
...
```liquidsoap
settings.decoder.decoders.set(["WAV", "AIFF", "PCM/BASIC", "MIDI", "IMAGE", "RAW AUDIO", "FFMPEG", "FLAC", "AAC", "MP4", "OGG", "MAD"])
```
...
### `video.add_text` implementation.

```liquidsoap
settings.video.add_text.set("sdl")
```

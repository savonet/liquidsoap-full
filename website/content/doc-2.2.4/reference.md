...
- `headers` (of type `[string * string]`, which defaults to `[("User-Agent", "Liquidsoap/2.2.4 (Unix; OCaml 4.14.1)")]`): Additional headers.
- `(unlabeled)` (of type `[string * string]`)
...
- `trim_url` (of type `bool`, which defaults to `true`): Trim input URL.
- `user_agent` (of type `string`, which defaults to `"Liquidsoap/2.2.4 (Unix; OCaml 4.14.1)"`): User agent.
- `(unlabeled)` (of type `{string}`): URL to decode.

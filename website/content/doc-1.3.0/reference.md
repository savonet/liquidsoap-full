...
#### get_process_lines
```
(?timeout:float,?env:[string],string)->[string]```
...
* `env` (`[string]` -- defaults to `[]`): Process environment
...
#### run_process
```
(?env:[string],?timeout:float,string)->
((string*string)*(string*string))```
...
* <code>env</code> (<code>[string]</code> -- defaults to <code>[]</code>): Process environment
...
#### test_process
```
(?timeout:float,?env:[string],string)->bool```

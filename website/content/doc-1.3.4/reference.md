...
* `timeout` (`float` -- defaults to `30.0`): Timeout for source connectionn.
* `user_agent` (`string` -- defaults to `"Liquidsoap/1.3.4 (Unix; OCaml 4.07.0)"`): User agent.
...
[("User-Agent","Liquidsoap/1.3.4 (Unix; OCaml 4.07.0)")]```
): Additional headers.
...
### output.youtube.live
```
(?id:string,url:'a,key:'b,
 source(audio='#c+1,video='#d+1,midi='#e))->
active_source(audio='#c+1,video='#d+1,midi='#e)```

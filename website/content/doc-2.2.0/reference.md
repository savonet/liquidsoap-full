...
- `(unlabeled)` (of type `source(audio=pcm('a), 'b)
where 'b is a set of tracks to be muxed into a source`): Source to be amplified.
...
```
(?id : string?, source(audio=ffmpeg.audio.raw('b),
 video=ffmpeg.video.raw('a))) -> source(audio=pcm('c), video=canvas('d))
```

Arguments:

- `id` (of type `string?`, which defaults to `"ffmpeg.raw.decode.audio_video"`)
- `(unlabeled)` (of type `source(audio=ffmpeg.audio.raw('b), video=ffmpeg.video.raw('a))`)
...
(?id : string?, format(audio=ffmpeg.audio.raw('a),
 video=ffmpeg.video.raw('b), 'c), source(audio=pcm('e), video=canvas('d))) ->
source(audio=ffmpeg.audio.raw('a), video=ffmpeg.video.raw('b))
```
...
```
(?id : string?, ?conservative : bool, ?duration : {float}, ?minimum : float,
 ?override_duration : string, ?persist_override : bool, ?width : float,
 ((
   {
     db_level : float,
...
```
(?fade_in : float, ?fade_out : float, source(audio=pcm('a), 'b),
 source(audio=pcm('a), 'b)) -> source(audio=pcm('a), 'b)
where
...
(?id : string?, ?duration : float, ?override_duration : string,
 ...
 ?default : ((source(audio=pcm('a), 'b), source(audio=pcm('a), 'b)) ->
             source(audio=pcm('a), 'b)),
 ?high : float, ?medium : float, ?margin : float, ?deduplicate : bool,
...
```
(?id : string, ?duration : float, ?override_duration : string,
 ?override_type : string, ?persist_overrides : bool, ?track_sensitive : bool,
...
- `override_type` (of type `string`, which defaults to `"liq_fade_type"`): Metadata field which, if present and correct, overrides the 'type' parameter for the current track.
...
```
(?id : string, ?duration : float, ?override_duration : string,
 ?override_type : string, ?persist_overrides : bool, ?override_skip : string,
 ?type : string, source(audio=pcm('a), 'b)) -> source(audio=pcm('a), 'b)
where 'b is a set of tracks to be muxed into a source
```
...
- `user_agent` (of type `string`, which defaults to `"Liquidsoap/2.2.0 (Unix; OCaml 4.14.1)"`): User agent.
- `(unlabeled)` (of type `{string}`): URL to decode.
...
(?id : string?, ?bind_address : string, ?clock_safe : bool,
 ...
 ?read_timeout : float?, ?start : bool, ?streamid : string?,
 ?write_timeout : float?) -> source('a)
```
...
(?id : string?, ?check_next : ((request) -> bool)?, ?prefetch : int,
 ...
 ?reload_mode : string, ?timeout : float, string) -> source('a)
```
...
```
(?id : string?, ?fallible : bool, ?prefetch : int, ?timeout : float, string) ->
source('a)
```
...
(?id : string?, ?averaging : float, ?buffer : float, ?fallible : bool,
 ...
 ?on_stop : (() -> unit), ?resample : bool, ?reset : bool, ?start : bool,
 source(audio=pcm('a), 'b)) -> source(audio=pcm('a), 'b)
```
...
(?id : string?, ?buffer_size : int, ?channels_matrix : string,
 ...
 ?options : [string * string], ?start : bool, source(audio=pcm('a), 'b)) ->
unit
```
...
```
(?id : string?, ?fallible : bool, ?on_start : (() -> unit),
 ?on_stop : (() -> unit), ?start : bool, source('a)) -> unit
```
...
```
(?id : string?, ?fallible : bool, ?flush : bool, ?on_reopen : (() -> unit),
 ?on_start : (() -> unit), ?on_stop : (() -> unit), ?reopen_delay : {float},
...
(?id : string?, ?append : bool, ?dir_perm : int, ?fallible : bool,
 ?flush : bool, ?on_close : ((string) -> unit), ?on_reopen : (() -> unit),
...
- `fallible` (of type `bool`, which defaults to `false`): Allow the child source to fail, in which case the output will be stopped until the source is available again.
- `on_file_change` (of type `(state : string, string) -> unit`, which defaults to `fun (~state=_,_) -> ()`): Callback executed when a file changes. `state` is one of: `"opened"`, `"closed"` or `"deleted"`, second argument is file path. Typical use: upload files to a CDN when done writing (`"close"` state and remove when `"deleted"`).
- `on_start` (of type `() -> unit`, which defaults to `{()}`): Callback executed when output starts.
- `on_stop` (of type `() -> unit`, which defaults to `{()}`): Callback executed when output stops.
...
(?id : string?, ?auth : ((address : string, string, string) -> bool)?,
 ...
 ?on_stop : (() -> unit), ?password : string?, ?port : int, ?start : bool,
...
(?id : string?, ?dir_perm : int, ?extra_tags : [string], ?fallible : bool,
 ...
 ?on_start : (() -> unit), ?on_stop : (() -> unit), ?perm : int,
 ?persist_at : string?, ?playlist : string, ?prefix : string,
 ?segment_duration : float, ?segments : int, ?segments_overhead : int,
...
(?id : string?, ?chunked : bool, ?connection_timeout : float,
 ...
 ?port : int, ?public : bool, ?send_icy_metadata : bool?, ?start : bool,
 ?timeout : float, ?transport : http_transport, ?url : string?,
 ?user : string?, format('a), source('a)) -> unit
```
...
(?id : string?, ?buflen : int, ?clock_safe : bool, ?device_id : int?,
 ?fallible : bool, ?latency : float?, ?on_start : (() -> unit),
 ?on_stop : (() -> unit), ?start : bool, source(audio=pcm('a), 'b)) -> unit
```
...
(?id : string?, ?chunked : bool, ?connection_timeout : float,
 ...
 ?on_stop : (() -> unit), ?password : string, ?port : int, ?public : bool,
...
(?id : string?, ?bind_address : string, ?connection_timeout : float?,
 ...
 ?payload_size : int, ?pbkeylen : int?, ?polling_delay : float, ?port : int,
 ?read_timeout : float?, ?start : bool, ?streamid : string?,
 ?write_timeout : float?, format('a), source('a)) -> unit
```
...
(?id : string?, ?fallible : bool,
 ...
 ?on_start : (() -> unit), ?on_stop : (() -> unit), ?restart_delay : float?,
 ?self_sync : bool, ?start : bool, url : string, format('a), source('a)) ->
unit
```
...
(?id : string?, ?cue_in_metadata : string, ?cue_out_metadata : string,
 ?on_cue_in : (() -> unit), ?on_cue_out : (() -> unit), source('a)) ->
source('a)
```
...
- `on_cue_in` (of type `() -> unit`, which defaults to `{()}`): Callback to execute on cue in
- `on_cue_out` (of type `() -> unit`, which defaults to `{()}`): Callback to execute on cue out
- `(unlabeled)` (of type `source('a)`)
...
### `runtime.jemalloc.mallctl.string`

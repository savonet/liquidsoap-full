...
* `timeout` (`float` -- defaults to `30.0`): Timeout for source connectionn.
* `user_agent` (`string` -- defaults to `"Liquidsoap/1.3.2 (Unix; OCaml 4.04.0)"`): User agent.
...
[("User-Agent","Liquidsoap/1.3.2 (Unix; OCaml 4.04.0)")]```
): Additional headers.
...
"/usr/local/lib/liquidsoap/1.3.2/extract-replaygain"```

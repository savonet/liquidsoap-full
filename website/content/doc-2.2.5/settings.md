...
```liquidsoap
settings.harbor.accept_timeout := 3.
```
...
##### Priority for the taglib metadata decoder

```liquidsoap
settings.request.metadata_decoders.priorities.taglib := 1
```
...
### How much data should be buffer from each track when computing crossfade transitions. This will be shorter if one of the track is shorter than that.
...
### How to proceed with amplify data when `"liq_amplify"` is set. One of: `"keep"` (prefer user-provided value), `"override"` (always override with computed value), `"ignore"` (ignore/suppress loudness adjustment).
...
```liquidsoap
settings.autocue.internal.metadata_override := ["liq_cue_in", "liq_cue_out", "liq_cross_start_next", "liq_fade_in", "liq_fade_in_delay", "liq_fade_out", "liq_fade_out_delay"]
```

#### Loudness target

```liquidsoap
settings.autocue.internal.lufs_target := -16.
```

#### Cue in threshold

```liquidsoap
settings.autocue.internal.cue_in_threshold := -34.
```

#### Cue out threshold

```liquidsoap
settings.autocue.internal.cue_out_threshold := -42.
```

#### Crossfade start threshold

```liquidsoap
settings.autocue.internal.cross_threshold := -7.
```
...
```liquidsoap
settings.decoder.ffmpeg.max_interleave_duration := 5.
```
...
```liquidsoap
settings.decoder.file_extensions.taglib := ["mp3", "ogg", "mkv", "wav", "flac", "mp4", "m4a"]
```
...
```liquidsoap
settings.decoder.mime_types.taglib := ["audio/mpeg", "audio/ogg", "video/ogg", "audio/wav", "audio/wave", "audio/flac", "audio/mp4", "video/mp4"]
```

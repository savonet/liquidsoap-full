...
##### Preferred codec to decode lead

```liquidsoap
settings.decoder.ffmpeg.codecs.lead := <value>
```

###### Available codecs to decode lead

```liquidsoap
settings.decoder.ffmpeg.codecs.lead.available := ["lead"]
```
...
##### Preferred codec to decode qoa

```liquidsoap
settings.decoder.ffmpeg.codecs.qoa := <value>
```

###### Available codecs to decode qoa

```liquidsoap
settings.decoder.ffmpeg.codecs.qoa.available := ["qoa"]
```
...
##### Preferred codec to decode vvc

```liquidsoap
settings.decoder.ffmpeg.codecs.vvc := <value>
```

###### Available codecs to decode vvc

```liquidsoap
settings.decoder.ffmpeg.codecs.vvc.available := ["vvc"]
```
...
#### Process logs through the main liquidsoap log facilities.
...
#### Include metadata when computing frame checksum

```liquidsoap
settings.frame.checksum.metadata := false
```

#### Include track marks when computing frame checksum.

```liquidsoap
settings.frame.checksum.track_marks := false
```
...
```liquidsoap
settings.log.syslog := false
```

#### Logging facility.

```liquidsoap
settings.log.syslog.facility := "DAEMON"
```

#### Logging level. One of: `"emergency"`, `"alert"`, `"critical"`, `"error"`, `"warning"`, `"notice"`, `"info"` or `"debug"`.

```liquidsoap
settings.log.syslog.level := "info"
```

#### Name of the program.

```liquidsoap
settings.log.syslog.program := "liquidsoap"
```
...
```liquidsoap
settings.request.metadata_decoders.recode.exclude.cover := true
```
...
```liquidsoap
settings.string.max_printable_length := 4096
```

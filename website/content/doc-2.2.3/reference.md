...
- `trim_url` (of type `bool`, which defaults to `true`): Trim input URL.
- `user_agent` (of type `string`, which defaults to `"Liquidsoap/2.2.3 (Unix; OCaml 4.14.1)"`): User agent.
- `(unlabeled)` (of type `{string}`): URL to decode.
...
(?id : string?, ?chunked : bool, ?connection_timeout : float,
 ...
 ?port : int, ?public : bool, ?register_telnet : bool,
...
(?id : string?, ?chunked : bool, ?connection_timeout : float,
 ...
 ?user : string?, ?icy_reset : bool, ?dj : (() -> string), ?aim : string,
 ?icq : string, ?irc : string, format('a), source('a)) -> unit
where 'a is a set of tracks to be muxed into a source
```

...
* `duration` (```
anything that is either float or ()->float```
 -- defaults to `0.5`): Duration of the RMS window (in seconds). A value <= 0, means that RMS computation should not be performed.
* `(unlabeled)` (`source(audio='#b,video='#c,midi='#d)`)
...
"/usr/local/lib/liquidsoap/1.2.0/extract-replaygain"```
...
* `(unlabeled)` (`'a`)
* `(unlabeled)` (`source(audio='#b+1,video='#c,midi='#d)`)
